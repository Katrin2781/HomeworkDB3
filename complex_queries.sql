...
select a.art_name, a2.alb_name  from artists a ,albums a2 ,album_artists aa 
where a.artistid = aa.artistid and a2.albumid = aa.albumid and a2.year_production != 2020;

--названия сборников, в которых присутствует конкретный исполнитель (выберите сами)
select distinct collect_name, collct_year_prod  from collections c
join collection_tracks ct on c.collectionid =ct.collectionid 
join tracks t on ct.trackid = t.trackid 
where t.artist =(select artistid from artists where art_alias='Дима Билан');

--название альбомов, в которых присутствуют исполнители более 1 жанра
select a.alb_name, a2.art_alias from albums a
...
select track_name, trackid  from tracks t 
where t.trackid not in(select trackid from collection_tracks ct2);

--исполнителя(-ей), написавшего самый короткий по продолжительности трек (теоретически таких треков может быть несколько)
select art_alias from artists a 
join tracks t ON a.artistid = t.artist 
where t.long_track =(select min(long_track) from tracks t2 );
